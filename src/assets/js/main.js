var Musa = function() {
	(this.body = document.querySelector("body")),
		...
		(this.sidebarMenuToggle = document.getElementById(
			"sidebar-menu-toggle"
		)),
		(this.allSidemenuLinks = document.querySelectorAll(
			"#sidebar-menu .menu-section .nav-item a"
		)),
		(this.allHasChildSidemenuLinks = document.querySelectorAll(
			"#sidebar-menu .menu-section .has-child a"
		)),
		...
		(this.handleWizardPreviousButton = document.getElementById(
			"ButtonPrevious"
		)),
		...
		(this.tableCollapsibleRow = document.getElementsByClassName(
			"row-collapsible"
		));
};
(Musa.prototype.init = function() {
	...
	(Musa.prototype.events = function() {
		musa.body.contains(this.menuToggle) &&
			this.menuToggle.addEventListener("click", musa.toggleMenu, !1),
			musa.body.contains(this.sidebarMenuToggle) &&
				this.sidebarMenuToggle.addEventListener(
					"click",
					musa.toggleMenu,
					!1
				),
			musa.body.contains(this.btnPrint) &&
				(this.btnPrint.onclick = function() {
					musa.printInvoice();
				});
		for (var t = 0; t < musa.allHasChildSidemenuLinks.length; t++)
			musa.allHasChildSidemenuLinks[t].addEventListener(
				"click",
				this.openSidebarSubMenu,
				!1
			);
		musa.body.contains(document.getElementById("ButtonPrevious")) &&
			musa.handleWizardPreviousButton.addEventListener(
				"click",
				musa.handleWizardPrevious,
				!1
			),
			musa.body.contains(musa.wizardNextButton) &&
				musa.wizardNextButton.addEventListener(
					"click",
					musa.handleWizardNext,
					!1
				);
	}),
	(Musa.prototype.materialFormFieldFilledState = function() {
		for (
			var t = document.querySelectorAll(".material-form .form-control"),
				e = 0;
			e < t.length;
			e++
		)
			t[e].onchange = function() {
				this.classList.add("filled"),
					"" === this.value && this.classList.remove("filled");
			};
	}),
	(Musa.prototype.wizardNavigation = function() {
		musa.body.contains(musa.wizardTab1) &&
			(musa.wizardTab1.onclick = function() {
				document
					.querySelector(".wizard-navigation .nav-item.active")
				...
				for (var t = 0; t < musa.stepContent.length; t++)
					musa.stepContent[t].setAttribute(
						"data-opened-step",
						"false"
					);
				document
					.getElementById("Step1")
					.setAttribute("data-opened-step", "true"),
					musa.wizardProgressBarNullWidth(),
					musa.wizardNextButton.setAttribute(
						"data-next-step",
						"Step2"
					),
					musa.handleWizardPreviousButton.setAttribute(
						"data-prev-step",
						""
					);
			}),
			musa.body.contains(musa.wizardTab2) &&
			...
			musa.body.contains(musa.wizardTab3) &&
			...
			musa.body.contains(musa.wizardTab4) &&
	...
	(Musa.prototype.handleWizardNext = function() {
		for (
			var t,
				e = musa.wizardNextButton.getAttribute("data-next-step"),
				s = 0;
			s < musa.stepContent.length;
			s++
		)
			musa.stepContent[s].setAttribute("data-opened-step", "false");
		document.getElementById(e).setAttribute("data-opened-step", "true"),
			"Step2" === e
				? (musa.wizardNextButton.setAttribute(
						"data-next-step",
						"Step3"
				  ...
				  (t = musa.wizardTab2).classList.add("active"),
				  t.previousElementSibling.classList.remove("active"),
				  t.previousElementSibling.classList.add("done"),
				  musa.wizardProgressBarOneThirdWidth())
				: "Step3" === e
				? (musa.wizardNextButton.setAttribute(
						"data-next-step",
				  ...
				  (t = musa.wizardTab3).classList.add("active"),
				  t.previousElementSibling.classList.remove("active"),
				  t.previousElementSibling.classList.add("done"),
				  musa.wizardProgressBarTwoThirdsWidth())
				: "Step4" === e &&
				  ...
				  musa.showWizardSubmitButton(),
				  (t = musa.wizardTab4).classList.add("active"),
				  t.previousElementSibling.classList.remove("active"),
				  t.previousElementSibling.classList.add("done"),
				  musa.wizardProgressBarFullWidth());
	}),
	(Musa.prototype.handleWizardPrevious = function() {
		for (
			var t,
				e = musa.handleWizardPreviousButton.getAttribute(
					"data-prev-step"
				),
				s = 0;
			s < musa.stepContent.length;
		...
		document.getElementById(e).setAttribute("data-opened-step", "true"),
			"Step1" === e
				? (musa.wizardNextButton.setAttribute(
				  ...
				  (t = musa.wizardTab1).classList.add("active"),
				  t.nextElementSibling.classList.remove("active"),
				  t.nextElementSibling.classList.remove("done"),
				  musa.wizardProgressBarNullWidth())
				: "Step2" === e
				? (musa.wizardNextButton.setAttribute(
						"data-next-step",
	...
	(Musa.prototype.hideWizardSubmitButton = function() {
		musa.wizardSubmit.classList.contains("show") &&
	...
	(Musa.prototype.openSidebarSubMenu = function() {
		for (
			var t = document.querySelectorAll(
					"#sidebar-menu .menu-section .side-menu li.has-child.opened"
				),
				e = 0;
			e < t.length;
			e++
			...
			null === t[e] ||
				s(this.parentNode, t[e]) ||
				t[e].classList.remove("opened");
		function s(t, e) {
		...
		this.parentNode.classList.contains("opened")
	...
	(Musa.prototype.toggleMenu = function() {
		...
		musa.body.classList.toggle("nav-sm"),
			...
			window.addEventListener("resize", function() {
				...
				t < 576 &&
					musa.body.classList.contains("nav-md") &&
					((musa.sidebar.style.height = e + "px"),
					musa.body.classList.contains("fixed-body")
		...
		s < 576 &&
			((musa.sidebar.style.height = a + "px"),
			musa.body.classList.contains("fixed-body")
				? musa.body.classList.remove("fixed-body")
				: musa.body.classList.add("fixed-body"));
	}),
	...
	(Musa.prototype.collapseTable = function() {
		for (var t = 0; t < musa.tableCollapsibleRow.length; t++)
			musa.tableCollapsibleRow[t].onclick = function() {
				var t = document.querySelectorAll(".row-collapsible.shown");
				this.classList.contains("shown")
					? this.classList.remove("shown")
					: this.classList.contains("shown") ||
					  (null != t[0] && t[0].classList.remove("shown"),
					  this.classList.add("shown"));
			};
	}),
	(Musa.prototype.rippleEffect = function() {
		for (
			var t = document.querySelectorAll(".ripple"),
				e = function(t) {
			...
			t[s].addEventListener("click", e, !1);
	}),
	...
	(Slider.prototype.slider = function() {
		...
		var e = document.querySelectorAll('input[data-attr="range-input"]'),
			s = function() {
				...
				if (
					(i.classList.contains("horizontal-slider") &&
						(a.style.left = "calc( +" + 100 * s + "% - 12px"),
					i.classList.contains("horizontal-slider-labeled-bottom") &&
						(a.style.top = "50px"),
					i.classList.contains("vertical-slider") &&
						(a.style.bottom = 100 * s + 38 + "px"),
					i.classList.contains("vertical-slider-labeled-right") &&
						(a.style.marginLeft = "78px"),
					i.classList.contains("rectangular-slider") &&
						(a.style.bottom = 100 * s + 38 + "px"),
			...
			a = function() {
				...
				null === localStorage.getItem("customizedColors") &&
					...
					i.classList.contains("horizontal-slider-labeled-bottom") &&
						(a.style.top = "70px"),
					i.classList.contains("rounded-slider") &&
						((a.style.color = t), (a.style.borderColor = t)),
					i.classList.contains("vertical-slider") &&
						((a.style.bottom = 100 * s + 20 + "px"),
						(a.style.borderColor = t)),
					i.classList.contains("vertical-slider-labeled-right") &&
						(a.style.marginLeft = "64px"),
					i.classList.contains("rectangular-slider") &&
						((a.style.bottom = 100 * s + 20 + "px"),
	...
	(Slider.prototype.sliderColor = function(t, e, s, a) {
		return (
			(this.style.background =
				...
				t +
				", #ddd))"),
			s.classList.contains("horizontal-slider") &&
				(a.style.borderColor = e),
			s.classList.contains("rounded-slider") &&
				((a.style.color = e), (a.style.borderColor = e)),
			s.classList.contains("vertical-slider") &&
				(a.style.borderColor = e),
			s.classList.contains("rectangular-slider") &&
				(a.style.backgroundColor = e),
			"input#" +
				this.getAttribute("id") +
				"." +
				this.className +
				"::-moz-range-track { background: -webkit-gradient(linear, left top, right top, color-stop(" +
				t +
				", " +
				e +
				"), color-stop(" +
